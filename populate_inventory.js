...
async function populateInventory() {
  let connection;
  try {
    console.log('🚀 Iniziando la pulizia e il popolamento dell\'inventario...');
    connection = await mysql.createConnection(config);
    console.log('✅ Connesso al database MySQL');

    await connection.beginTransaction();

    console.log('🗑️ Eliminando tutti i prezzi di vendita esistenti...');
    await connection.execute('DELETE FROM selling_prices');
    
    console.log('🗑️ Eliminando tutti gli articoli di inventario esistenti...');
    await connection.execute('DELETE FROM inventory');
    
    console.log('🗑️ Eliminando tutte le modifiche esistenti...');
    await connection.execute('DELETE FROM modifications');
    
    console.log('🗑️ Eliminando tutti gli ordini esistenti...');
    await connection.execute('DELETE FROM orders');
    
    console.log('🗑️ Eliminando tutti i log attività esistenti...');
    await connection.execute('DELETE FROM activity_log');

    console.log('➕ Inserendo i 25 articoli corretti nell\'inventario...');
    for (const item of inventoryData) {
      ...
      console.log(`✅ Inserito: ${item.name} (ID: ${inventoryId})`);
    }

    await connection.commit();
    console.log('');
    console.log('🎉 INVENTARIO PULITO E POPOLATO CON SUCCESSO!');
    console.log(`📊 Inseriti ${inventoryData.length} articoli unici`);
    console.log('💰 Prezzi di vendita configurati');
    console.log('🗑️ Tutti i duplicati rimossi');
    console.log('');
    ...
    console.error('❌ Errore durante il popolamento dell\'inventario:', error);
    process.exit(1);
  } finally {
    if (connection) {
