...
router.post("/reset-db", async (req, res) => {
  try {
    ...
    await db.run(`DROP TABLE IF EXISTS activity_log`);
    await db.run(`DROP TABLE IF EXISTS daily_earnings`);
    await db.run(`DROP TABLE IF EXISTS selling_prices`);
    await db.run(`DROP TABLE IF EXISTS modifications`);
    await db.run(`DROP TABLE IF EXISTS employee_deductions`);
    await db.run(`DROP TABLE IF EXISTS orders`);
    await db.run(`DROP TABLE IF EXISTS salary_history`);
    await db.run(`DROP TABLE IF EXISTS inventory`);
    await db.run(`DROP TABLE IF EXISTS employees`);
    await db.run(`DROP TABLE IF EXISTS bot_status`);
